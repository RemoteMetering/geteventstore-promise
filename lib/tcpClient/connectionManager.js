...
export default {
	async create(config, onConnected, isSubscription = false) {
		...
		if (!connectionPool || isSubscription) {
			...
			const factory = {
				async create() {
					const client = createConnection(config, (config.gossipSeeds || `${config.protocol}://${config.hostname}:${config.port}`), config.generateConnectionName ? await config.generateConnectionName() : undefined);
