...
// Without one it is the config's operations pool or, when it has none, every subscription pool of it.
const findPoolsToClose = (config, connectionName) => {
  if (connectionName) {
    const pool = findSubscriptionPoolByName(connectionName) || _configPools.get(config);
    return pool ? [pool] : [];
  }
