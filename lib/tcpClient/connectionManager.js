...
// and a name that matches none closes nothing. Only a subscription closing itself passes a name,
// so falling back to the operations pool would drain it under concurrent reads and writes.
// Without a name it is the config's operations pool or, when it has none, every subscription pool of it.
const findPoolsToClose = (config, connectionName) => {
  ...
  return _subscriptionPools.filter((pool) => pool.config === config);
};
