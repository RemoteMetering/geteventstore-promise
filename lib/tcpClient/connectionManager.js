...
// the hot create() path (onConnected is only ever set for subscriptions). Subscription
// pools share their config so they are kept in a separate list, looked up by connectionName.
...
export default {
	...
	getPool(config) {
		return async (connectionName) => {
			const connectionPool = _configPools.get(config) || _subscriptionPools.find(pool => pool.connectionName && pool.connectionName === connectionName);
