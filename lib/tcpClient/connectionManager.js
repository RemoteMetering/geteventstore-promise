...
// connection name, because a subscription is closed through the name its client reported.
const findPool = (config, connectionName) =>
  _configPools.get(config) ||
  _subscriptionPools.find(
    (pool) => pool.config === config || (pool.connectionName && pool.connectionName === connectionName)
  );
