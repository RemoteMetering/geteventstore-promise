...
export default (config) => async(streamName, events, options) => {
	...
	const transaction = await connection.startTransaction(streamName, options.expectedVersion, config.credentials);

	const eventChunks = _.chunk(eventsToWrite, options.transactionWriteSize);
	await Promise.each(eventChunks, events => transaction.write(events));
