...
export default (config) => async function* (streamName, chunkSize, startPosition, resolveLinkTos) {
	...
	try {
		...
		while (true) {
			const result = await connection.readStreamEventsForward(streamName, position, chunkSize, resolveLinkTos, config.credentials);
			if (result.error) throw new Error(result.error);

			for (const event of mapEvents(result.events)) {
				yield event;
			}
