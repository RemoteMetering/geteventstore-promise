...
export default (config) => async (writes) => {
  ...
  const requests = writes.map((write, index) => {
    ...
    assert(Array.isArray(write.events), `${baseErr}Events should be an array for write at index ${index}`);
  ...
  const result = jsonSafe(await connection.multiStreamAppend(requests));
