...
// commitPosition is non-enumerable, so it is added back here explicitly.
function mappedEventToJson() {
  const plain = { ...this };
  if (typeof this.commitPosition === 'bigint') plain.commitPosition = String(this.commitPosition);
  if (plain.position) plain.position = toPlainJson(plain.position);
  return plain;
}

// commitPosition is a bare BigInt, which cannot carry a toJSON of its own the way position does.
// Kept non-enumerable, a spread or Object.assign copy of the event drops it rather than carrying a
// BigInt that makes JSON.stringify throw. ev.commitPosition still reads it, and position.commit
// holds the same value on events that have a position.
const setCommitPosition = (mappedEvent, commitPosition) =>
  Object.defineProperty(mappedEvent, 'commitPosition', {
...
export const mapEvent = (ev) => {
  ...
  if (!event) {
    ...
    if (ev.commitPosition !== undefined) setCommitPosition(deletedEvent, ev.commitPosition);
  ...
  if (ev.commitPosition !== undefined) setCommitPosition(mappedEvent, ev.commitPosition);
