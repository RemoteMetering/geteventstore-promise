...
const toFromRevision = (fromEventNumber) => {
  ...
  const isRevision =
    typeof fromEventNumber === 'bigint' ||
    Number.isSafeInteger(fromEventNumber) ||
    (typeof fromEventNumber === 'string' && /^-?\d+$/.test(fromEventNumber));
  if (!isRevision) throw new Error(`${baseErr}'fromEventNumber' not valid: ${String(fromEventNumber)}`);
