...
export default (config, direction) => async function* (startPosition, count, resolveLinkTos, filter) {
	...
	for await (const event of connection.readAll({ direction: `${direction}s`, maxCount: count, fromPosition: _startPosition, resolveLinkTos, filter })) {
