...
export default (config) =>
  async (streamName, onEventAppeared, onDropped, resolveLinkTos = false) => {
    assert(streamName, `${baseErr}Stream Name not provided`);

    const connection = await connectionManager.getOrCreate(config);
    const subscription = await connection.subscribeToStream(streamName, { fromRevision: 'end', resolveLinkTos });
