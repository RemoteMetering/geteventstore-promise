...
export default (config) => async (streamName, eventType, data, metaData, options) => {
	...
	options = options || {};
	options.expectedVersion = toExpectedState(options.expectedVersion);
	...
	try {
		const result = await connection.appendToStream(streamName, [event], { streamState: options.expectedVersion, batchAppendSize: options.batchAppendSizeInBytes });
