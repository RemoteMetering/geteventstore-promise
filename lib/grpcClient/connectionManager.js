	...
	const connectionPool = {
		...
		pool: genericPool.createPool({
			async create() {
				...
				const client = EventStoreDBClient.connectionString(`${config.protocol}://${config.gossipSeeds ? config.gossipSeeds.map(seed => `${seed.hostname}:${seed.port}`).join(',') : `${config.auth}@${config.host}:${config.port}`}?tls=${config.useSslConnection}${connectionName ? `&connectionName=${connectionName}` : ''}`);
				// client.projec
				if (isSubscription) {
