	...
	const connectionPool = {
		...
		pool: genericPool.createPool({
			async create() {
				...
				// client.createProjection(prjectionName, theQuery, { trackEmittedStreams?: boolean });
				// client.updateProjection(projectionName, theQuery, { emitEnabled?: boolean; });
				// client.deleteProjection(projectionName, {    deleteEmittedStreams?: boolean; deleteStateStream?: boolean; deleteCheckpointStream?: boolean;});
				// client.enableProjection(projectionName, { none });
				// client.disableProjection(projectionName, { none });
				// client.abortProjection(projectionName, { none });
				// client.resetProjection()
				// client.getProjectionResult(projectionName, { partition?: string; });
				// client.getProjectionState(projectionName, { partition?: string; });
				// client.getProjectionStatus(projectionName, { none });
