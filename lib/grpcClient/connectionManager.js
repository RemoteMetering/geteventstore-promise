	...
	const connectionPool = {
		...
		pool: genericPool.createPool({
			async create() {
				...
				const client = KurrentDBClient.connectionString(`${config.protocol}://${config.gossipSeeds ? config.gossipSeeds.map(seed => `${seed.hostname}:${seed.port}`).join(',') : `${config.auth}@${config.host}:${config.port}`}?tls=${config.useSslConnection}${config.tlsCAFile ? `&tlsCAFile=${config.tlsCAFile}` : ''}${connectionName ? `&connectionName=${connectionName}` : ''}`);
				if (isSubscription) {
