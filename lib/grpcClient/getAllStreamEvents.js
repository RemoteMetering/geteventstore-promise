...
export default (config) => async (streamName, chunkSize, startPosition, resolveLinkTos) => {
	...
	async function getNextChunk(startPosition) {
		...
		for await (const event of connection.readStream(streamName, { direction: 'forwards', maxCount: chunkSize, fromRevision: !isNaN(startPosition) ? BigInt(startPosition) : startPosition, resolveLinkTos })) {
			events.push(event);
			hasNewEvents = true;
		}

		if (hasNewEvents) {
			return getNextChunk(events[events.length - 1].event.revision + 1);
		}
