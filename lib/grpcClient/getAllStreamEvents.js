...
export default (config) => async (streamName, chunkSize, startPosition, resolveLinkTos) => {
	...
	async function getNextChunk(startPosition) {
		...
		const fromRevision = typeof startPosition === 'bigint' ? startPosition : !isNaN(startPosition) ? BigInt(startPosition) : startPosition;
		for await (const event of connection.readStream(streamName, { direction: 'forwards', maxCount: chunkSize, fromRevision, resolveLinkTos })) {
			events.push(event);
			hasNewEvents = true;
		}
