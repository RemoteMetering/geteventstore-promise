...
describe('TCP Client - Test Connection', () => {
	...
	it('Should default to only one connection with no pool options provided', async function() {
		...
		const _instances = require('../tcpClient/connectionManager')._instances;
		assert(_instances, 'Instances expected');
		const instance = find(_instances, { config });
		assert(instance, 'Instance expected');
		assert.equal(1, instance.pool._allObjects.size);
	});
	...
	it('Should fill up pool connections to provided max', async function() {
		...
		const instance = find(_instances, { config });
		assert(instance, 'Instance expected');
		assert.equal(7, instance.pool._allObjects.size);
	});

	it('Should close all streams', async function() {
		this.timeout(60 * 1000);
		const config = JSON.parse(JSON.stringify(tcpConfig));
		const client = eventstore.tcp(config);
		...
		const _instances = require('../tcpClient/connectionManager')._instances;
		assert(_instances, 'Instances expected');
		assert.equal(0, _instances.length);
	});
});