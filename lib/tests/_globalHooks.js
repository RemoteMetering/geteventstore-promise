...
const addClusterContainers = async () => new Promise((resolve, reject) => {
	const proc = spawn('docker-compose', ['--file', path.join(__dirname, 'support', 'cluster', 'docker-compose.yml'), 'up', '-d'], {
		cwd: undefined,
		stdio: ['ignore', 'ignore', process.stderr]
	});
...
const removeClusterContainers = async () => new Promise((resolve, reject) => {
	const proc = spawn('docker-compose', ['--file', path.join(__dirname, 'support', 'cluster', 'docker-compose.yml'), 'down'], {
		cwd: undefined,
		stdio: ['ignore', 'ignore', process.stderr]
	});
