...
// Shared constructor validation for all three clients. Only the cluster-aware clients (TCP and
// gRPC) accept gossipSeeds in place of hostname and port, so the HTTP client opts out and always
// has hostname and port validated.
export default (config, baseErr, { allowGossipSeeds = true } = {}) => {
  assert(config, `${baseErr}config not provided`);
  if (allowGossipSeeds && config.gossipSeeds) {
