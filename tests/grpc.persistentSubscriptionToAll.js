...
describe('gRPC Client - Persistent Subscription to $all', () => {
	it('Should receive events written to $all after the subscription starts', async function () {
		...
		function onEventAppeared(sub, ev) {
			sub.ack(ev);
			if (ev.eventType === eventType) processedEventCount++;
		}
	...
	it('Should only receive filtered events when a filter is supplied', async function () {
		...
		assert(received.every(t => t === eventType), `expect only ${eventType} events, got ${received.join(', ')}`);
	...
	it('getToAllSubscriptionsInfo should list the created group', async function () {
		...
		assert(all.some(s => s.groupName === groupName), 'expect the created group in the list');
